...
public class SentimentManager {
	...
	// For now just iterate through the variations of words (nouns, verbs..etc
	// etc)
	...
	// Accessor
	public static SentimentManager instance(String swnFilePath) {
		if (instance == null) {
			instance = new SentimentManager(swnFilePath);
		}
		return instance;
	}

	// Initialize the sentiwordnet file into a dictionary
	protected SentimentManager(String swnFilePath) {
		try {
			...
			for (Iterator<String> iterator = temp.iterator(); iterator
				...
				score /= sum;
				// if (word == null) {
				// System.out.println("Arrrrrrrrrrrrrrrrrgggggggg"); }
